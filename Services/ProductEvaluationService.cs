    ...
    IOrderTransactionManager transactionManager) : IProductEvaluationService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var selected = ValidateDimensions(request);
        ...
        await transactionManager.ExecuteAsync(async transaction =>
        {
            ...
            await evaluationRepository.InsertAsync(CreateEvaluation(order, detail, selected), transaction);
        });
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await transactionManager.ExecuteAsync(async transaction =>
        {
            ...
            var details = (await orderRepository.GetDetailsAsync(orderId, transaction)).ToList();
            if (details.Count == 0)
                throw new GroupBBusinessException("订单商品不存在");
            if (details.Any(item => !string.Equals(item.ReceiptStatus, "RECEIVED", StringComparison.Ordinal)))
                throw new GroupBBusinessException("整单确认收货后才能评价");

            var evaluatedIds = await evaluationRepository.GetEvaluatedOrderDetailIdsAsync(
                details.Select(item => item.OrderDetailId), transaction);
            var pendingDetails = details.Where(item => !evaluatedIds.Contains(item.OrderDetailId)).ToList();
            if (pendingDetails.Count == 0)
                throw new GroupBBusinessException("该订单已完成评价");
    ...
    public Task<IReadOnlyList<ProductEvaluation>> GetByOrderDetailIdsAsync(IEnumerable<string> orderDetailIds) =>
        evaluationRepository.GetByOrderDetailIdsAsync(orderDetailIds);

    private static HashSet<string> ValidateDimensions(ProductEvaluationRequest request)
    {
    ...
    private static ProductEvaluation CreateEvaluation(
        BizOrder order,
