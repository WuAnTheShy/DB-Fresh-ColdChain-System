...
{
    public class GroupC_RefundManager : GroupC_IRefundManager
    {
        ...
        public GroupC_RefundManager(IUnitOfWork uow,PromoterRepository promoterRepository, GroupC_PromoterManager promoterManager, 
        ...
        public async Task<Result> Refund(GroupC_RefundRequest refundRequest) //整体处理退款函数
        {
            ...
            try
            {
                ...
                if (DateTime.Now - _orderInfo.CommSettlementDate >= TimeSpan.FromDays(14))
                {
                    await _uow.RollbackAsync();
                    _result.IsSuccess = false;
                    _result.ErrorMessage = "该订单已过可退期14天，无法退款！";
                    return _result;
                }
                ...
                else
                {
                    ...
                    if (refundRequest.RefundQty > _detailInfo.Quantity) //退款数额大于下单数额
                    {
                        await _uow.RollbackAsync();
                        _result.IsSuccess = false;
                        _result.ErrorMessage = "退期数量大于购买数量，请重新输入退款数量！";
                        return _result;
                    }
                    //计算退款金额
        ...
        private bool RefundRollbackMoney(string? promoterID, decimal totalCommission, decimal goodsAmount)  //退款回滚与退款信息记录
        {
            ...
            GroupC_CrmPromoter _promoterInfo = new GroupC_CrmPromoter(); 
        ...
        {
            GroupC_FinRefund _finRefund = new GroupC_FinRefund();
