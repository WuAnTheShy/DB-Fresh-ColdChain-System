...
{
    /// <summary>介绍标题（可空）</summary>
    public string? Title { get; set; }

    /// <summary>介绍段落（顺序即展示顺序）</summary>
    public List<PromoterIntroSection> Sections { get; set; } = new();

    /// <summary>是否无任何内容（标题为空且无段落）</summary>
    public bool IsEmpty()
    {
    ...
    /// <summary>规范化：清理标题与段落中的空值，删除无效图片地址与完全空的段落。</summary>
    ...
    /// <summary>取纯文本形态（标题 + 各段文字，段与段换行分隔），供跨组消费者端简介使用。</summary>
...
{
    /// <summary>段落文字（可空，空则表示本段为“纯图片”段）</summary>
    public string? Text { get; set; }

    /// <summary>段落图片（相对站点路径或 http(s) 地址，可空则表示本段为“纯文字”段）</summary>
    ...
    /// <summary>规范化：清理文字、过滤非法图片地址并去重、控制图片数量上限。</summary>
    public void Normalize()
    {
    ...
    /// <summary>图片地址白名单：相对路径必须以 / 开头（禁止协议相对、../、反斜杠、盘符冒号），或 http/https 链接。</summary>
...
{
    /// <summary>是否有图文内容（无标题且无段落时为 false，界面提示“团长暂未撰写推文”）</summary>
    public bool HasIntro { get; set; }

    /// <summary>推文标题（可空）</summary>
    public string? Title { get; set; }

    /// <summary>推文段落（按顺序展示；每段可只有文字、只有图片或图文并存）</summary>
    public List<PromoterIntroSection> Sections { get; set; } = new();
}
