...
public interface ISupplierService
{
    ...
    // 查询某供应商已建立货物的商品列表（含售价/保质期，供应商详情页用）
    ...
    // 变更供应商状态（Active/Pending/Disabled/Rejected），含状态流转校验。
    // 审核通过时可同时登记信用分（creditLevel 为 null 表示不改动信用分）。
    Task<ApiResponse> SetSupplierStatusAsync(string supplierId, string targetStatus, int? creditLevel = null);
}
