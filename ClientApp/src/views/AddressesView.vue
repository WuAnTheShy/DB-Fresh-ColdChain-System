...
const officialProvinces = ref([])
const regionLoading = ref(true)

function appendCurrentOption(options, currentName, level) {
  if (!currentName || options.some(option => option.name === currentName)) return options
  return [...options, { code: `legacy-${level}`, name: currentName, level, type: '历史地址', children: [] }]
}

function normalizeCities(province) {
  if (!province) return []
  const children = Array.isArray(province.children) ? province.children : []
  if (province.type === '直辖市') {
    return [{ code: province.code, name: province.name, level: 2, type: '直辖市', children }]
  }
  if (!children.length) {
    return [{ code: province.code, name: province.name, level: 2, type: province.type, children: [province] }]
  }
  return children.map(area => area.level === 2
    ? area
    : { code: area.code, name: area.name, level: 2, type: area.type, children: [area] })
}

const provinceOptions = computed(() => appendCurrentOption(officialProvinces.value, form.province, 1))
const selectedProvince = computed(() => provinceOptions.value.find(option => option.name === form.province))
const cityOptions = computed(() => appendCurrentOption(normalizeCities(selectedProvince.value), form.city, 2))
const selectedCity = computed(() => cityOptions.value.find(option => option.name === form.city))
const districtOptions = computed(() => {
  const children = Array.isArray(selectedCity.value?.children) ? selectedCity.value.children : []
  const options = children.length ? children : selectedCity.value ? [selectedCity.value] : []
  return appendCurrentOption(options, form.district, 3)
})

function changeProvince() { form.city = ''; form.district = '' }
function changeCity() { form.district = '' }

async function loadRegions() {
  regionLoading.value = true
  try {
    const response = await fetch(regionDataUrl)
    if (!response.ok) throw new Error('行政区划数据读取失败')
    const data = await response.json()
    officialProvinces.value = Array.isArray(data.provinces) ? data.provinces : []
    if (!officialProvinces.value.length) throw new Error('行政区划数据为空')
  } catch (requestError) {
...
<template>
  <div class="store-container page-space addresses-page">
    ...
    <section v-if="editorOpen" class="address-editor">
      ...
      <form class="address-form" @submit.prevent="saveAddress"><label class="wide"><span>收货人</span><input v-model.trim="form.receiverName" class="form-control" maxlength="50" required /></label><label class="wide"><span>手机号码</span><input v-model.trim="form.phone" class="form-control" pattern="1[0-9]{10}" maxlength="20" required /></label><label><span>省份</span><select v-model="form.province" class="form-select" required :disabled="regionLoading" @change="changeProvince"><option disabled value="">{{ regionLoading ? '正在加载行政区划' : '请选择省份' }}</option><option v-for="province in provinceOptions" :key="province.code" :value="province.name">{{ province.name }}</option></select></label><label><span>城市</span><select v-model="form.city" class="form-select" required :disabled="regionLoading || !form.province" @change="changeCity"><option disabled value="">请选择城市</option><option v-for="city in cityOptions" :key="city.code" :value="city.name">{{ city.name }}</option></select></label><label><span>区县</span><select v-model="form.district" class="form-select" required :disabled="regionLoading || !form.city"><option disabled value="">请选择区县</option><option v-for="district in districtOptions" :key="district.code" :value="district.name">{{ district.name }}</option></select></label><label class="wide"><span>详细地址</span><input v-model.trim="form.detailAddress" class="form-control" maxlength="200" required /></label><label class="address-default-check wide"><input v-model="form.isDefault" class="form-check-input" type="checkbox" />设为默认收货地址</label><div class="address-form-actions wide"><button class="btn btn-outline-secondary" type="button" @click="editorOpen = false">取消</button><button class="btn btn-buy" type="submit" :disabled="saving || regionLoading"><span v-if="saving" class="spinner-border spinner-border-sm"></span><Save v-else :size="17" />保存地址</button></div></form>
    </section>
...
.address-form .form-control, .address-form .form-select { padding: 8px 2px; border: 0; border-bottom: 1px solid #c8cdca; border-radius: 0; background-color: transparent; box-shadow: none; transition: border-color .15s ease; }
.address-form .form-select { padding-right: 28px; background-position: right 2px center; }
.address-form .form-control:focus, .address-form .form-select:focus { border-bottom-color: #111; outline: 0; box-shadow: none; }
.address-form .form-select:disabled { border-bottom-color: #e2e5e3; background-color: transparent; color: #9aa19d; }
