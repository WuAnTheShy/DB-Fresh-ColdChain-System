  ...
  <div v-if="leader" class="leader-detail-page">
    <section class="leader-profile-band">
      ...
      <div class="store-container leader-profile-content">
        ...
        <button class="btn leader-follow-button" :class="followed ? 'btn-light' : 'btn-buy'" type="button"
          :disabled="followPending" @click="handleFollow">
          <Heart :size="17" :fill="followed ? 'currentColor' : 'none'" />{{ !isAuthenticated ? '登录后关注' : followed ?
            '取消关注' : '关注团长' }}
        </button>
      </div>
    </section>

    <div v-if="followError" class="store-container leader-follow-alert alert alert-warning" role="alert">{{ followError
      }}</div>

    <div class="store-container leader-stat-row">
      <div>
        <PackageCheck :size="20" /><span><strong>{{ leaderProducts.length }}</strong><small>正在带货</small></span>
      </div>
      <div>
        <UsersRound :size="20" /><span><strong>{{ followed ? '已关注' : '未关注' }}</strong><small>当前关注状态</small></span>
      </div>
      <div>
        <BadgeCheck :size="20" /><span><strong>已认证</strong><small>平台团长资质</small></span>
      </div>
    </div>
    ...
    <div class="store-container home-section">
      ...
      <p v-if="leaderProducts.length" class="list-end-tip">到底了~</p>
      <div v-else class="store-empty">
        <strong>该团长暂无在团商品</strong>
        <span>团长暂时没有上架商品，去看看其他好物吧</span>
        <RouterLink class="btn btn-outline-secondary" to="/search">浏览全部商品</RouterLink>
      </div>
    </div>
  </div>
...
.leader-stat-row>div:last-child {
  border-right: 0;
}
...
.leader-stat-row strong {
  color: var(--ink);
  font-size: 14px;
}

.leader-stat-row small {
  color: var(--muted);
  font-size: 9px;
}
