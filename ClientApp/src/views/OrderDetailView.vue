<script setup>
import { AlertTriangle, BadgeCheck, Ban, CheckCircle2, ChevronLeft, Clock3, CreditCard, MapPin, PackageCheck, RefreshCw, RotateCcw, Snowflake, Truck } from '@lucide/vue'
      ...
      <div class="order-detail-layout">
        <div>
          ...
          <section class="order-consumer-section delivery-section">
            <div class="consumer-section-title"><Truck :size="21" /><div><h2>冷链配送</h2><p>不同供应商的商品可能拆分为多个包裹配送</p></div></div>
            <div v-if="!detail.packages?.length" class="delivery-status-row"><span class="delivery-icon"><Truck :size="20" /></span><div><strong>订单已进入备货流程</strong><small>发货后将在此展示最新配送状态</small></div></div>
            <article v-for="parcel in detail.packages" :key="parcel.packageNumber" class="parcel-card">
              <header><div><strong>包裹 {{ parcel.packageNumber }}</strong><small>{{ parcel.itemIds.length }} 件订单明细 · {{ temperatureName(parcel.logistics.packageTemperature) }}</small></div><StatusBadge :status="parcel.logistics.statusCode" /></header>
              <div v-if="parcel.logistics.hasException" class="parcel-exception"><AlertTriangle :size="17" /><span><strong>配送异常</strong>{{ parcel.logistics.exceptionMessage }}</span></div>
              <dl class="parcel-facts">
                <div><dt>承运商</dt><dd>{{ parcel.logistics.carrierName || '待分配' }}</dd></div>
                <div><dt>运单号</dt><dd>{{ parcel.logistics.trackingNo || '待生成' }}</dd></div>
                <div><dt><Snowflake :size="13" />运输温区</dt><dd>{{ temperatureName(parcel.logistics.packageTemperature) }}</dd></div>
                <div><dt><Clock3 :size="13" />预计送达</dt><dd>{{ date(parcel.logistics.estimatedArrivalAt) }}</dd></div>
              </dl>
              <div v-if="parcel.logistics.events?.length" class="parcel-events">
                <div v-for="event in parcel.logistics.events" :key="event.eventId" class="parcel-event" :class="{ danger: event.isTemperatureException }">
                  <span></span><div><header><strong>{{ event.statusName }}</strong><time>{{ date(event.occurredAt) }}</time></header><p>{{ event.description }}</p><small>{{ event.location || '位置待更新' }}<b v-if="event.temperatureCelsius != null">{{ Number(event.temperatureCelsius).toFixed(1) }}℃</b></small></div>
                </div>
              </div>
        ...
        <aside>
          ...
          <section class="order-side-section"><h2>金额明细</h2><dl><div><dt>商品金额</dt><dd>{{ money(detail.order.totalAmount) }}</dd></div><div><dt>团购优惠</dt><dd>-{{ money(detail.order.discountAmount) }}</dd></div><div v-if="detail.order.pointsUsed"><dt>积分抵扣（{{ detail.order.pointsUsed }}积分）</dt><dd>-{{ money(detail.order.pointsDiscountAmount) }}</dd></div><div><dt>冷链运费</dt><dd>{{ money(detail.order.freightAmount) }}</dd></div><div v-if="detail.freightQuote" class="freight-rule"><dt>计费依据</dt><dd>{{ detail.freightQuote.ruleSummary }}<small>{{ detail.freightQuote.province }} {{ detail.freightQuote.city }} {{ detail.freightQuote.district }}</small></dd></div><div class="order-pay-total"><dt>实付金额</dt><dd>{{ money(detail.order.finalAmount) }}</dd></div></dl></section>
          <div class="order-detail-actions"><RouterLink v-if="detail.order.orderStatus === 'PENDING_PAYMENT' && detail.order.checkoutBatchId" class="btn btn-buy" :to="`/payment/${detail.order.checkoutBatchId}`"><CreditCard :size="17" />支付整个结算批次</RouterLink><RouterLink v-if="['PAID', 'SHIPPED', 'COMPLETED', 'REFUNDING'].includes(detail.order.orderStatus)" class="btn btn-outline-danger" :to="`/orders/${id}/refund`"><RotateCcw :size="16" />申请退款</RouterLink><button v-if="detail.canCancel" class="btn btn-outline-danger" type="button" :disabled="acting" @click="runAction(() => api.cancelOrder(id), '订单已取消')"><Ban :size="17" />取消订单</button><button class="btn btn-outline-secondary" type="button" @click="loadOrder"><RefreshCw :size="16" />刷新状态</button></div>
        </aside>
      </div>
    </template>
  </div>
...
.parcel-card { margin-top: 11px; border: 1px solid var(--line); background: #fbfcfb; }
.parcel-card > header { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 14px; border-bottom: 1px solid var(--line); background: #f4f7f5; }
.parcel-card > header > div { display: flex; flex-direction: column; gap: 2px; }
.parcel-card > header small { color: var(--muted); font-size: 9px; }
.parcel-exception { display: flex; gap: 8px; padding: 10px 14px; border-bottom: 1px solid #f1b8b8; background: #fff1f1; color: #a52d2d; }
.parcel-exception span { display: flex; flex-direction: column; font-size: 10px; }
.parcel-facts { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px 18px; margin: 0; padding: 12px 14px; }
.parcel-facts > div { display: flex; justify-content: space-between; gap: 8px; }
.parcel-facts dt { display: flex; align-items: center; gap: 4px; color: var(--muted); font-size: 9px; font-weight: 500; }
.parcel-facts dd { margin: 0; font-size: 10px; text-align: right; }
.parcel-events { padding: 2px 14px 12px; }
.parcel-event { display: grid; grid-template-columns: 10px minmax(0, 1fr); gap: 8px; padding-top: 10px; }
.parcel-event > span { position: relative; width: 8px; height: 8px; margin-top: 4px; border-radius: 50%; background: var(--brand); }
.parcel-event > span::after { position: absolute; top: 8px; left: 3px; width: 1px; height: calc(100% + 32px); background: #cfd9d4; content: ''; }
.parcel-event:last-child > span::after { display: none; }
.parcel-event.danger > span { background: var(--danger); }
.parcel-event header { display: flex; justify-content: space-between; gap: 8px; }
.parcel-event time { color: var(--muted); font-size: 8px; }
.parcel-event p { margin: 2px 0; font-size: 10px; }
.parcel-event small { display: flex; gap: 8px; color: var(--muted); font-size: 9px; }
.parcel-event b { color: var(--brand); }
.parcel-event.danger b { color: var(--danger); }
.parcel-pending { padding: 11px 14px; border-top: 1px solid var(--line); color: var(--muted); font-size: 9px; }
...
.order-side-section .freight-rule { align-items: flex-start; }
.freight-rule dd { max-width: 180px; }
.freight-rule small { display: block; margin-top: 3px; color: var(--muted); font-size: 8px; }
...
@media (max-width: 767.98px) {
  ...
  .parcel-facts { grid-template-columns: 1fr; }
}
</style>
