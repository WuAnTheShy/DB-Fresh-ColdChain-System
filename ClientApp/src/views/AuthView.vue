  ...
  <div class="auth-page">
    <section class="auth-intro">
      <div class="auth-intro-content">
        <span class="auth-kicker"><ShieldCheck :size="16" /> 鲜邻团账户</span>
        <h1>从产地冷链，到家门口的新鲜</h1>
      ...
      <form class="auth-form" @submit.prevent="submit">
        ...
        <label v-else>
          ...
          <span class="password-field">
            ...
            <input v-else v-model="loginForm.password" class="form-control" :type="showPassword ? 'text' : 'password'" autocomplete="current-password" minlength="8" maxlength="100" placeholder="至少8个字符" required />
            <button type="button" :aria-label="showPassword ? '隐藏密码' : '显示密码'" @click="showPassword = !showPassword"><EyeOff v-if="showPassword" :size="18" /><Eye v-else :size="18" /></button>
          </span>
        </label>
...
<style scoped>
.auth-page { display: grid; width: min(980px, calc(100% - 30px)); min-height: 620px; grid-template-columns: minmax(0, 1.05fr) minmax(390px, .95fr); margin: 34px auto 8px; overflow: hidden; border-radius: 12px; background: #fff; box-shadow: 0 10px 35px rgba(15, 17, 17, .15); }
.auth-intro { position: relative; display: flex; align-items: flex-end; overflow: hidden; padding: 48px; background: linear-gradient(145deg, rgba(10,39,31,.9), rgba(0,95,115,.86)), url('../assets/categories/vegetable-tofu.jpg') center / cover; color: #fff; }
.auth-intro::after { position: absolute; inset: 0; background: radial-gradient(circle at 80% 10%, rgba(254,189,105,.32), transparent 38%); content: ''; }
.auth-intro-content { position: relative; z-index: 1; }
.auth-kicker { display: inline-flex; align-items: center; gap: 7px; color: #ffd99d; font-size: 12px; font-weight: 750; }
.auth-intro h1 { max-width: 430px; margin: 16px 0 14px; font-size: clamp(32px, 4vw, 48px); font-weight: 800; line-height: 1.14; }
.auth-intro p { max-width: 430px; margin: 0; color: #d8e6e0; font-size: 14px; line-height: 1.75; }
.auth-intro ul { display: grid; gap: 8px; margin: 25px 0 0; padding: 0; list-style: none; color: #f1f6f4; font-size: 12px; }
.auth-intro li::before { margin-right: 8px; color: #febd69; content: '✓'; font-weight: 800; }
.auth-panel { padding: 38px 42px; }
.auth-tabs { display: grid; grid-template-columns: 1fr 1fr; margin-bottom: 31px; border-bottom: 1px solid var(--line); }
.auth-tabs button { min-height: 44px; border: 0; border-bottom: 3px solid transparent; background: transparent; color: var(--muted); font-weight: 700; }
.auth-tabs button.active { border-bottom-color: var(--amber-hover); color: var(--ink); }
.auth-heading { display: flex; align-items: center; gap: 12px; margin-bottom: 22px; }
.auth-heading-icon { display: inline-flex; width: 46px; height: 46px; flex: 0 0 46px; align-items: center; justify-content: center; border-radius: 50%; background: #e8f2ee; color: var(--brand); }
.auth-heading h2 { margin: 0; font-size: 24px; font-weight: 800; }
.auth-heading p { margin: 4px 0 0; color: var(--muted); font-size: 11px; }
.auth-alert { margin-bottom: 16px; font-size: 12px; }
.auth-form { display: grid; gap: 15px; }
.auth-form label { display: flex; flex-direction: column; gap: 6px; }
.auth-form label > span:first-child { color: #39443f; font-size: 11px; font-weight: 750; }
.auth-form label small { color: #8a928e; font-weight: 500; }
...
@media (max-width: 767.98px) {
  ...
  .auth-panel { padding: 25px 21px 30px; }
