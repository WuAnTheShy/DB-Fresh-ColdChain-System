  ...
  <div class="store-container page-space cart-page">
    <div class="cart-title">
      <div>
        ...
        <h1>购物车</h1>
      </div><span>共 {{ cartCount }} 件商品</span>
    </div>

    <div v-if="cartItems.length" class="cart-layout">
      <div class="cart-groups">
        <div class="cart-select-toolbar">
          <label><input class="form-check-input" type="checkbox" :checked="allSelected"
              @change="setAllCartSelected($event.target.checked)" />全选</label>
        ...
        <section v-for="group in groups" :key="group.leader.id" class="cart-leader-group">
          <header @click="setLeaderCartSelected(group.leader.id, !group.selected)">
            ...
            <span class="cart-leader-area">{{ group.leader.area }}</span>
          </header>
          <article v-for="item in group.items" :key="`${item.productId}-${item.leaderId}`" class="cart-item"
            @click="setCartItemSelected(item.productId, !item.selected)">
            <label class="cart-item-check" @click.stop><input class="form-check-input" type="checkbox"
            ...
            <div class="cart-item-main">
              ...
              <small>
                <Truck :size="13" />{{ item.product.delivery }}
              </small>
            </div>
            <div class="cart-unit-price">¥{{ item.product.price.toFixed(2) }}</div>
            <QuantityStepper :model-value="item.quantity" :max="item.product.stock" @click.stop
            ...
            <button class="cart-remove" type="button" title="移出购物车" @click.stop="removeFromCart(item.productId)">
              <Trash2 :size="18" />
            </button>
          </article>
      ...
      <aside class="cart-summary">
        ...
        <dl>
          <div>
            <dt>已选商品</dt>
            <dd>{{ selectedCartCount }} 件</dd>
          </div>
          <div>
            <dt>商品小计</dt>
            <dd>¥{{ selectedCartSubtotal.toFixed(2) }}</dd>
          </div>
          <div>
            <dt>预计运费</dt>
            <dd>结算时分别计算</dd>
        ...
        <RouterLink v-if="selectedCartCount" class="btn btn-buy w-100 checkout-button" to="/checkout">去结算
          <ArrowRight :size="18" />
        </RouterLink>
        <button v-else class="btn btn-secondary w-100 checkout-button" type="button" disabled>请先选择商品</button>
        <small>
          <LockKeyhole :size="14" />价格和库存将在提交时由服务器确认
        </small>
      </aside>
    </div>

    <div v-else class="store-empty cart-empty">
      <ShoppingCart :size="42" /><strong>购物车还是空的</strong><span>浏览正在销售的生鲜商品</span>
...
.cart-leader-group>header svg {
  color: var(--brand);
}

.cart-leader-group>header>.cart-leader-area {
  margin-left: auto;
  color: var(--muted);
  font-size: 9px;
}
...
.cart-item {
  display: grid;
  grid-template-columns: 24px 94px minmax(160px, 1fr) 80px 120px 90px 34px;
  gap: 12px;
  align-items: center;
  padding: 16px;
  border-bottom: 1px solid var(--line);
...
.cart-leader-group>footer strong {
  color: #745910;
}
...
.cart-summary {
  ...
  border-radius: 6px;
  background: #fff;
  box-shadow: 0 3px 10px rgba(23, 33, 29, .07);
}
...
@media (max-width: 1199.98px) {
  .cart-item {
    grid-template-columns: 24px 80px minmax(140px, 1fr) 110px 80px 34px;
  }
...
@media (max-width: 767.98px) {
  .cart-item {
    grid-template-columns: 22px 70px minmax(0, 1fr) 34px;
    gap: 9px;
    align-items: start;
  }
