...
function clampQuantity(item) {
  ...
  selection.quantity = Math.min(Math.max(1, Number(selection.quantity || 1)), Number(item.quantity ?? 1))
}
        ...
        <form @submit.prevent="submitRefund">
          <fieldset :disabled="hasPending || submitting">
            ...
            <div class="refund-products">
              <article v-for="item in orderDetail.details" :key="item.orderDetailId" :class="{ selected: selections[item.orderDetailId]?.selected }">
                <label class="product-choice"><input v-model="selections[item.orderDetailId].selected" type="checkbox" /><span><strong>{{ item.productName }}</strong><small>{{ money(item.unitPrice) }} × {{ item.quantity }} 件</small></span></label>
                <label class="product-quantity"><span>退货数量</span><input v-model.number="selections[item.orderDetailId].quantity" class="form-control" type="number" min="1" :max="item.quantity" :disabled="!selections[item.orderDetailId].selected" @input="clampQuantity(item)" /></label>
              </article>
            </div>
            ...
            <button class="btn btn-buy submit-refund" type="submit" :disabled="!remark || !selectedEntries.length || hasPending || submitting"><span v-if="submitting" class="spinner-border spinner-border-sm"></span>提交平台审核</button>
          </fieldset>
        </form>
      </main>
