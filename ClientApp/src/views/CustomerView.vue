...
<template>
  <div class="store-container page-space profile-page">
    ...
    <template v-else-if="profile">
      ...
      <section class="profile-metrics"><RouterLink to="/orders"><PackageSearch :size="22" /><span><strong>我的订单</strong><small>查看订单进度</small></span><ChevronRight :size="17" /></RouterLink><RouterLink to="/coupons"><TicketPercent :size="22" /><span><strong>优惠券</strong><small>领取和使用</small></span><ChevronRight :size="17" /></RouterLink><div><Coins :size="22" /><span><strong>{{ profile.customer.points }} 积分</strong><small>累计消费 {{ money(profile.customer.totalSpent) }}</small></span></div><RouterLink to="/messages"><Bell :size="22" /><span><strong>消息中心</strong><small>订单和退款提醒</small></span><ChevronRight :size="17" /></RouterLink></section>
      <div class="profile-content-grid">
        <section class="account-section member-level-section"><div class="account-section-head"><div><h2>会员等级</h2><p>每月 1 日按上月最后一天前已完结订单的累计消费金额定级</p></div></div><div class="level-progress"><strong>{{ profile.memberLevel?.levelName || '基础会员' }}</strong><span>累计消费 {{ money(profile.customer.totalSpent) }}</span><small>等级权益将于下一个定级日更新</small></div><div v-if="profile.memberLevelHistory?.length" class="level-history"><div v-for="item in profile.memberLevelHistory.slice(0, 5)" :key="item.historyId"><span>{{ String(item.settlementMonth).slice(0, 7) }} 定级</span><strong>{{ item.levelName }}</strong><em>累计 {{ money(item.qualifiedSpent) }}</em></div></div><div v-else class="inline-empty">暂无历史定级记录，将在每月 1 日生成</div></section>
...
.member-level-section { grid-column: 1 / -1; }.level-progress { display: grid; grid-template-columns: 1fr auto; gap: 6px 16px; align-items: center; padding: 14px; background: #f3f8f6; }.level-progress strong { color: var(--brand); font-size: 19px; }.level-progress span { font-size: 11px; font-weight: 700; }.level-progress small { grid-column: 1 / -1; color: var(--muted); font-size: 9px; }.level-history { margin-top: 12px; }.level-history > div { display: grid; grid-template-columns: 1fr auto auto; gap: 14px; padding: 9px 2px; border-top: 1px solid var(--line); font-size: 10px; }.level-history span { color: var(--muted); }.level-history strong { color: var(--brand); }.level-history em { color: var(--muted); font-style: normal; }
