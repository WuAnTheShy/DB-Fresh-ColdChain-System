...
internal static class FreightAggregationScenarioTests
{
    ...
    private static FreightItemDto Item(string id, int quantity = 1, string supplier = "SUP1") => new()
    {
        ProductID = id, SupplierID = supplier, Quantity = quantity
    };
    ...
    private static ColdChainLogisticsService Service(List<InvProduct> products, params LogFreightTemplate[] rules) => new(
        ...
        FinancialProxy.Create<IGoodsRepository>((method, args) => method.Name == nameof(IGoodsRepository.GetAsync)
            ? Task.FromResult<InvGoods?>(new InvGoods
            {
                ProductID = (string)args![0]!, SupplierID = (string)args[1]!, SalePrice = 50m
            })
            : throw new NotSupportedException(method.Name)),
    ...
    private static async Task SamePackageAsync()
    {
        ...
        var quote = await Quote(service, Request(Item("P1"), Item("P2")));
    ...
    private static async Task SeparatePackagesAsync()
    {
        foreach (var differentSupplier in new[] { false, true })
        {
            ...
            var quote = await Quote(service, Request(Item("P1"), Item("P2", supplier: differentSupplier ? "SUP2" : "SUP1")));
    ...
    private static async Task DuplicateItemsAsync()
    {
        ...
        var split = await Quote(service, Request(Item("P1"), Item("P1", 2)));
        var merged = await Quote(service, Request(Item("P1", 3)));
    ...
    {
        var request = Request(Item("P1", supplier: ""));
